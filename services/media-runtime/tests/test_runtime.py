...
class MediaRuntimeTests(unittest.TestCase):
    ...
    def test_pixabay_handler_uses_source_adapter_and_returns_private_metadata_headers(self) -> None:
        ...
        self.assertNotIn("=", response.headers["x-pixabay-track-title-base64"])
        self.assertNotIn("=", response.headers["x-pixabay-query-base64"])
        self.assertNotIn("=", response.headers["x-pixabay-filename-base64"])
        self.assertEqual(encode_pixabay_header("Theme"), "VGhlbWU")
