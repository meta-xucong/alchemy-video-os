...
**底层复用规则（不可放宽）**：只移植参考仓库中已有的代码、字段、参数、校验和流程，平台只做输入/输出、权限、工作区、对象存储、错误映射、任务幂等和公开脱敏等薄壳适配；不在已有来源能力上另写平行算法、阈值、协议、静默回退或 UI 语义。任何修改、删减、替换都必须以固定 commit 中的具体文件、符号或规则为参照，并登记改动原因和行为回归测试。参考仓库没有安全对应实现时，默认保持禁用或 fail-closed，不得自行补造逻辑。这里的“薄壳”不取消平台自己的身份、工作区、Control API、持久化、队列和审计边界；这些边界不能反过来改变来源媒体/内容逻辑的含义。
...
### 2.2 原仓库优先与最小薄壳适配（底层规则）

以下规则适用于所有参考仓库、所有新章节和所有后续重构：

**每做一步，都必须回顾该步骤是否确实移植了原仓库已有代码和逻辑；凡不能指向原仓库具体来源的新增内容一律停止，不得自行造内容。**

1. **先源后实现**：编码前先定位固定 commit 中的原始文件、函数、类型、字段、参数和测试；没有来源定位不得开始新增行为。
2. **直接移植优先**：来源已有实现与平台契约兼容时，直接移植原代码或保持等价调用顺序；平台只在边界处做薄 mapper/serializer/adapter。
3. **禁止平行逻辑**：不得为替代来源实现而自行发明算法、评分、阈值、时长策略、自动改写、协议版本、回退路径或 UI 状态含义。
4. **修改删减可追溯**：每一个修改、删减、重命名或替换都要写明来源文件/符号、保留内容、平台适配点、舍弃原因和行为测试；不能只写“优化”或“兼容”。
5. **来源缺口即阻断**：来源没有安全实现、能力未认证或语义无法证明时，保持禁用、等待或 fail-closed；不以“先跑起来”作为自造逻辑的理由。
6. **平台边界不越界**：workspace/auth/API/queue/storage/audit 是平台必须拥有的外壳，但外壳不得把来源的媒体、内容或 Provider 语义改成另一套规则。
7. **多实现同语义**：内存、数据库、Runtime、兼容读取器和测试夹具必须遵循同一来源语义；不能只修一条实现制造假绿测试。
8. **先最小后扩展**：优先修正错误路径并明确不支持边界；只有来源证据、契约、定向行为测试和审计齐全后，才允许扩大能力范围。

当前 C12.4/C12.5 的来源追踪、跨仓库冲突审计和逐项执行清单以《AI企业内容生产平台_多源仓库逐项迁移矩阵与冲突审计开发方案.md》为准；冻结的《AI企业内容生产平台_C12.4-C12.5最小化源仓库适配修改方案.md》和《AI企业内容生产平台_C12.4-C12.5逐项源仓库迁移与验证开发文档.md》只保留历史实施证据，不再授权新增实现。新方案不能覆盖本文件、领域/API 契约或本地 MVP 的更高优先级规则。
...
### 3.1.1 当前自动旁白使用口径（2026-09-01）

当前产品场景是“自动生成视频并自动生成/保留其音频”，不要求用户上传旁白或样音。样音必须由已选定的 Provider/native 音轨或显式选择的来源 TTS 在服务端生成，再经过人工试听批准；批准后的样音不能直接充当正式 `NarrationAssetVersion`。通用参考图片、企业资料、Logo、MUSIC 资产上传仍按既有契约工作；`NARRATION_SAMPLE`、`USER_SOURCE_AUDIO` 仅保留为兼容既有资产与角色隔离事实，不是当前自动旁白的输入前提。

音频 owner 必须先由固定来源能力和 profile 事实确定：已认证 Grok native 音轨保留在 MP4 中且不再调用 TTS；只有操作者明确选择时才走 OpenMontage `TTSSelector` 对应的 Doubao 路径；没有来源证据时保持 `UNAVAILABLE`/`BLOCKED`。不得默认注入 Piper、自动 native→Doubao fallback、自动审批或自造语速/时长修正。具体迁移顺序和实时对照以《AI企业内容生产平台_自动生成音频与视频匹配正式使用开发文档.md》为准；默认 `VIDEO_PROVIDER=mock` 与本地 CI 规则不变。
...
以上顺序不是建议而是硬门：若来源已有可用实现，不能先写平台自有实现再“参考”来源；若必须偏离来源，先记录具体冲突和最小适配理由。修改和删减以来源代码、文档或测试为标准，不以本地猜测、视觉效果或一次偶然成功作为依据。来源未覆盖的能力必须显式标记为 `UNAVAILABLE`、`DEFERRED` 或 `BLOCKED`，不得包装成已完成。
