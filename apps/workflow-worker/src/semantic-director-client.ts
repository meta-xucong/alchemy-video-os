...
const directorSystemPrompt = [
  ...
  "READY is allowed only when every dialogue, reference use, source fact, duration, and required decision cites matching immutable evidence and is executable.",
  "The platform checks citation identity, exact spans, ordering, and capability bounds only; it cannot prove semantic entailment or source completeness for you.",
].join("\n");
...
export const createProvenanceCheckedSemanticDirectorFromEnv = (
  ...
  return new ProvenanceCheckedSemanticDirector(new OpenAiCompatibleSemanticDirector({
    baseUrl,
    apiKey,
    model,
