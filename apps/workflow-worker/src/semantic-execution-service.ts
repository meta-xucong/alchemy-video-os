 ...
 *
 * Semantic understanding has exactly one owner: the LLM director wrapped by
 * ProvenanceCheckedSemanticDirector. This executor freezes source material,
 * validates citation identity/order/bounds, projects the LLM decision, and
 * persists immutable output. It does not prove semantic entailment, complete
 * source coverage, or rendered-media correctness.
 */
export class SemanticCreativePlanningExecutor {
  ...
  private compileSegment(
    ...
    return compileVideoPrompt({
      // Provider prompt source is the LLM visual projection. The authored
      // brief remains frozen separately in CanonicalSourceBundle/source_hash.
