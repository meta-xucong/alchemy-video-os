...
import { createCanonicalSourceBundle } from "@alchemy-video/creative-planning/semantic-director";
import {
...
test("provenance-checked semantic director sends the complete canonical bundle and accepts only structurally valid evidence references", async () => {
  const requests: Array<{ url: string; init: RequestInit; body: Record<string, unknown> }> = [];
  const director = createProvenanceCheckedSemanticDirectorFromEnv({
...
test("semantic director configuration is explicit and unsafe endpoints are rejected", () => {
  assert.throws(() => createProvenanceCheckedSemanticDirectorFromEnv({}),
