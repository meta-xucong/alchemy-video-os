...
// The fixture follows the real three-key seam: segment count, durations and
// dialogue ownership are supplied by the injected director, not by a local
// deterministic skeleton.
const segmentFixtureFor = (context: LlmFreeformPlanningContext) => {
  const minimum = Math.max(1, context.minimumSegments ?? 1);
  const count = Math.max(minimum, Math.ceil(context.targetDurationSeconds / context.durationBounds.maxDurationSeconds));
  const durations: number[] = [];
  let remaining = context.targetDurationSeconds;
  for (let index = 0; index < count; index += 1) {
    const slotsAfter = count - index - 1;
    const duration = Math.min(
      context.durationBounds.maxDurationSeconds,
      Math.max(context.durationBounds.minDurationSeconds, remaining - slotsAfter * context.durationBounds.minDurationSeconds),
    );
  ...
  return durations.map((duration, index) => ({
    ...
    visual_prompt: `自由视觉 ${index + 1}：只描述本段画面并在明确终点收束。`,
    dialogue_line_sequences: context.dialogueLines.length > 0
      ? context.dialogueLines.map((_line, lineIndex) => lineIndex + 1).filter((lineIndex) => lineIndex % count === index)
      : [],
  }));
};

const freeformPlanner = () => new LlmFreeformPromptPlanningModel(async (context) =>
  segmentFixtureFor(context));
...
test("CreativePlanningExecutor injects freeform visual prompts while preserving authored dialogue", async () => {
  ...
  let captured: CreativePlanningDraft | undefined;
  let receivedContext: LlmFreeformPlanningContext | undefined;
  const planner = new LlmFreeformPromptPlanningModel(async (context) => {
    receivedContext = context;
    return segmentFixtureFor(context);
  });
  const executor = new CreativePlanningExecutor({
    async completeCreativePlan(input) {
      captured = input.draft;
  ...
  // 完整原文、受保护台词与活动时长边界进入上下文；分段决定由 LLM 负责。
  assert.deepEqual(receivedContext?.dialogueLines, ["必须逐字保留。"]);
  assert.equal(receivedContext?.sourceText, freeformBrief.sourceText);
  assert.deepEqual(receivedContext?.durationBounds, { minDurationSeconds: 8, maxDurationSeconds: 15 });
  assert.equal(receivedContext?.minimumSegments, undefined);
  assert.deepEqual(captured.shotSpecs.map((shot) => shot.durationSeconds), [15, 15]);
  assert.ok(captured.shotSpecs.every((shot) => !("visualPrompt" in shot)));
  let visualPromptPartCount = 0;
  for (const [index, promptPackage] of captured.promptPackages.entries()) {
    ...
    const freeformVisualPrompt = `自由视觉 ${index + 1}：只描述本段画面并在明确终点收束。`;
    if (generatedPromptParts.some((part) => part === freeformVisualPrompt)) visualPromptPartCount += 1;
    assert.equal(promptPackage.prompt.includes(freeformVisualPrompt), true);
  }
  assert.equal(visualPromptPartCount, 1);
...
test("CreativePlanningExecutor consumes a verified semantic planner fixture without widening each segment source", async () => {
  ...
  if (!captured) return;
  assert.equal(captured.shotSpecs.length, 2);
  const prompts = captured.promptPackages?.map((promptPackage) => promptPackage.prompt) ?? [];
  // 每段视觉补充只进入对应的 prompt，不回灌其它段。
  for (const [index, prompt] of prompts.entries()) {
    assert.ok(prompt.includes(`自由视觉 ${index + 1}`));
    assert.ok(prompts
      .filter((_other, otherIndex) => otherIndex !== index)
      .every((other) => !prompt.includes(other)));
  }
});
...
test("injected semantic planner keeps each JSON-sidecar source ordered through the production snapshot factory", async () => {
  ...
  for (const [index, promptPackage] of captured.promptPackages.entries()) {
    ...
    const laterOrEarlierSource = captured.shotSpecs
      .filter((_shot, sourceIndex) => sourceIndex !== index)
...
test("injected semantic planner keeps compacted segment sidecars ordered through JSON and the production snapshot", async () => {
  ...
  }, freeformPlanner(), compiler, undefined, undefined, profile.audioOwner,
  ...
  const createSnapshot = createProductionTaskRunInputSnapshotFactory("sub2api");
  for (const [index, promptPackage] of captured.promptPackages.entries()) {
    const sourcePrompt = captured.shotSpecs[index]!.narrativeGoal;
    const persistedSidecar = JSON.parse(JSON.stringify(promptPackage.capabilitySnapshot)) as Record<string, unknown>;
