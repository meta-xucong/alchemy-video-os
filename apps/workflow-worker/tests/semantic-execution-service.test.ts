...
import { VerifiedSemanticDirector } from "@alchemy-video/creative-planning";
...
test("real semantic executor persists exact dialogue, canonical references, and no legacy motion facts", async () => {
  ...
  const director = new VerifiedSemanticDirector({
  ...
  assert.doesNotMatch(promptPackage!.prompt, /PLATFORM_OWNED_|Motion timeline|PROP CONTINUITY CONTRACT/);
...
test("real semantic executor blocks when a source asset cannot be resolved canonically", async () => {
  const director = new VerifiedSemanticDirector({ async decide(bundle) { return decisionFor(bundle.source_hash); } });
  const executor = new SemanticCreativePlanningExecutor({
