...
export class MockVideoTaskExecutor {
  constructor(
    ...
    private readonly options: Readonly<{
      ...
      /** Read-only Sub2API usage owner for a shared Provider API key. */
      videoUsageUserId?: number;
    }> = {},
  ) {}
  ...
  async execute(input: { workspaceId: string; taskRunId: string }) {
    ...
    try {
      ...
      if (!providerRequestId
        && this.options.videoUsageUserId !== undefined
        && inputSnapshot.billing?.billing_rule.usagePricing
        && inputSnapshot.billing.external_user_id !== this.options.videoUsageUserId) {
        return this.store.failTaskRun({
          workspaceId: input.workspaceId,
          taskRunId: taskRun.id,
          providerAttemptId: attempt.id,
          ...
          code: "PROVIDER_REJECTED",
          message: "The configured video Provider account does not match the current AISelf account.",
          retryable: false,
          now: new Date(),
        });
      }
  ...
  private async resolveBillingUsage(
    ...
    try {
      usage = await this.options.videoUsage.getUsage({
        externalUserId: this.options.videoUsageUserId ?? billing.external_user_id,
