...
import { parseSub2ApiVideoUsageUserId } from "./video-usage-config.js";
...
let videoUsageUserId: number | undefined;
if (process.env.VEYRA_CREDIT_ENABLED === "true") {
  ...
  videoUsageUserId = parseSub2ApiVideoUsageUserId(process.env.SUB2API_VIDEO_USAGE_USER_ID);
...
const executor = new MockVideoTaskExecutor(store, runtime.provider, storage, {
  ...
  ...(videoUsageUserId === undefined ? {} : { videoUsageUserId }),
});
