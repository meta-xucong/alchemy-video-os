...
class SuccessfulCreditPort implements CreditPort {
  ...
  async debit(input: Parameters<CreditPort["debit"]>[0]) {
    this.debitCalls += 1;
    this.debitUserIds.push(input.externalUserId);
    return {
      externalUserId: input.externalUserId,
      amount: input.amount,
      balanceAfter: "98",
...
test("a shared Provider account mismatch is rejected before submission", async () => {
  const { store, workspaceId, taskRunId } = await prepareTask(true, "grok-imagine-video-1.5");
  const provider = new MockVideoProvider({ fixtureBytes: await createMockMp4Fixture() });
  const executor = new MockVideoTaskExecutor(store, provider, createInMemoryStoragePort(), {
    videoUsageUserId: 7,
  ...
  assert.equal(result?.status, "FAILED");
  assert.equal(result?.error?.code, "PROVIDER_REJECTED");
  assert.match(result?.error?.message ?? "", /does not match/u);
  assert.equal(provider.submitCount, 0);
});

test("a matching shared Provider account keeps native usage and current-user debit aligned", async () => {
  const { store, workspaceId, taskRunId } = await prepareTask(true, "grok-imagine-video-1.5");
  const provider = new MockVideoProvider({ fixtureBytes: await createMockMp4Fixture() });
  const credit = new SuccessfulCreditPort();
  const billingExecutor = new VideoBillingExecutor(credit, new InMemoryBillingAttemptStore(store), {
  ...
  const usageLookupUserIds: number[] = [];
  const executor = new MockVideoTaskExecutor(store, provider, createInMemoryStoragePort(), {
    billingExecutor,
    videoUsageUserId: 20260909,
    videoUsage: {
      async getUsage(input) {
        usageLookupUserIds.push(input.externalUserId);
        return { providerRequestId: input.providerRequestId, model: "grok-imagine-video-1-5-preview", actualCost: "0.12" };
      },
    },
  });
  ...
  assert.equal(result?.status, "SUCCEEDED");
  assert.deepEqual(usageLookupUserIds, [20260909]);
  assert.deepEqual(credit.debitUserIds, [20260909]);
});
