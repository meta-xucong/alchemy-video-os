 ...
 * the protected loopback boundary and decodes its private response metadata.
 */
...
/**
 * Source-runtime transport. Local full-stack runs keep Media Runtime on
 * loopback, preserving the source Python tool's network behavior there.
 */
export class HttpPixabayMusicClient implements PixabayMusicPort {
  ...
  constructor(input: { runtimeUrl: string; token: string; fetcher?: Fetcher }) {
    ...
    if (
      ...
      || !["127.0.0.1", "localhost", "[::1]", "::1"].includes(runtimeUrl.hostname)
      || runtimeUrl.username
      || runtimeUrl.password
      || runtimeUrl.search
      || runtimeUrl.hash
      ...
      throw new Error("MEDIA_RUNTIME_URL must be a loopback http endpoint and MEDIA_RUNTIME_TOKEN is required.");
    }
