...
function Stop-ManagedLocalProcesses {
  ...
  $managed = Get-CimInstance Win32_Process | Where-Object {
    ...
    $commandLine.Contains($runtimeText) -or $commandLine.Contains(".codex-longrun\local-acceptance\")
  }

  foreach ($process in $managed) {
    Write-Host ("Stopping managed local process PID {0}: {1}" -f $process.ProcessId, $process.Name)
...
$localEnvironment = [ordered]@{
  ...
  S3_PUBLIC_ENDPOINT = "http://127.0.0.1:9002"
  S3_REGION = "us-east-1"
...
if ($VideoProvider -eq "sub2api") {
  if ($ReferenceDeliveryOrigin) {
    $referenceDeliveryPublicOrigin = $ReferenceDeliveryOrigin.Trim()
