...
关联文档：`AI企业内容生产平台_C11.3动作节拍与时间轴提示词开发设计.md`、`AI企业内容生产平台_长叙事自动编排与连续成片设计.md`、`AI企业内容生产平台_长叙事后端领域与编排开发设计.md`、`AI企业内容生产平台_领域模型与API事件契约.md`

本规范是平台级通用规则。以后任何项目、题材、Provider、时长或参考素材组合，都必须先经过本规范定义的“叙事点 → 动作节拍 → 生成片段 → 最终成片”编排。项目之间只允许改变规划结果，不允许改变这条主链路。
...
`MotionBeat` 是叙事点和生成片段之间的私有执行层。它描述一个在同一场景和连续时间内可执行的主要动作，不直接对应 Provider 调用。
...
1. 一个动作必须有可观察的动词和结束姿态；抽象情绪必须转为表情、视线、呼吸或身体状态。
2. 一个生成片段默认承载 2-4 个主要 MotionBeat；动作、场景或时间冲突时必须重新分组。
3. 时间轴必须无重叠、无空档，并覆盖完整生成片段。
4. 旧版没有 MotionBeat 的 StoryboardRevision 使用兼容编译器；新 revision 必须冻结动作计划。

### 2.3 GenerationSegment：生成片段
...
一个生成片段可以承载多个叙事点和多个 MotionBeat，但必须形成一个可连续拍摄的视觉动作或场景单元。不能把互不相关的场景强行塞进同一片段。

### 2.4 VideoVersion：最终成片
...
| 总时长 | 默认生成片段策略 | 说明 |
| --- | --- | --- |
| 15 秒 | 1 × 15 秒 | 段内用时间轴完成 2-4 个动作；仅明确编辑性场景切换可拆为 2 段 |
| 30 秒 | 2 × 15 秒 | 默认最少调用方案 |
| 60 秒 | 4 × 15 秒 | 按叙事转折微调 |
| 90 秒 | 6 × 15 秒 | 可按场景边界调整 |
| 120 秒 | 8 × 15 秒 | 长内容按章节组织 |

实际拆分不得机械套用表格。算法必须先生成 MotionBeat，再在安全范围内选择最少且足够表达内容的生成片段数；动作密度、道具换手、运镜词和叙事点数量本身不得增加 Provider 调用：
...
每个 segment 默认承载 2-4 个主要 MotionBeat
超过动作复杂度阈值时按场景、因果和结束姿态重新分组
```
...
每个新 GenerationSegment 必须拥有私有动作时间轴。编译顺序为：

```text
参考图职责 -> 全局连续性锁 -> 项目事实 -> MotionBeat 时间轴 -> 音频提示 -> 禁止项与结束状态
```

时间轴应优先采用 2-3 秒建立、3-6 秒主要动作、至少 2 秒结果落点的节奏。动作密集内容可拆成更多主片段；文学解释、品牌事实和情绪说明不得直接变成无边界动作列表。
...
- `HANDOFF_FIRST_FRAME`：上一段动作、地点或画面承接优先；后续片段必须把前段通过 QC 的交接帧作为第 0 位视觉输入，并在可用时继续携带用户上传参考图稳定人物、服装、产品和场景。
- `TEXT_TRANSITION`：允许明显转场或 B-roll，不提供视觉连续性保证。

如果 Provider 不支持“literal 首帧字段 + 独立参考图字段”同时发送，系统不能静默丢弃任何一类视觉事实。默认受控策略是使用有序多参考输入：第 0 位为交接帧，第 1 位起为用户参考图，最多 7 张，并在 PromptPackage 中明确“第一张参考图是开场交接帧”。该策略降低断裂风险，但仍不得承诺模型逐帧无缝。
...
Segment N 成功
  -> 提取尾部/终端交接帧
  -> QC 通过
  -> Segment N+1 以交接帧为第 0 位视觉输入，并追加用户参考图后才可入队
```
...
在 C12.1 中，技术 QC 通过后，系统对每个相邻主片段的“前段尾帧 -> 后段首帧”执行受控语义检查。它只判断人物身份、服装、场景、构图和动作方向是否存在明显断裂，不把评估器原始输出、评分或模型字段暴露给用户。

```text
PASS              -> 直接按成片计划合成
BLEND             -> 使用受限画面/音频淡变
BRIDGE_REQUIRED   -> 最多生成或编排一个 1-3 秒本地自动转场，并记录修复计划
UNAVAILABLE       -> 直切并标记“衔接检查未完成”，不创建自动修复
FAILED            -> 直切并标记需关注，保留已完成片段和旧成片
```

自动转场是 `TransitionRepair`，不是 `NarrativeBeat` 或新的主 `GenerationSegment`。它不能改变用户界面中的预计主片段数，也不能无界增加时长、成本或 Provider 调用。只有评估明确要求 `BLEND` 或 `BRIDGE_REQUIRED` 时才创建修复；C12.1 的本地转场不增加 Provider TaskRun；每个边界最多一次自动修复，整次制作的自动修复上限在确认 `ProductionRun` 时冻结。最终合成必须通过有界重叠或裁切维持目标时长容差；无法满足时长守卫时不得标记成功。
...
1. “你的内容将被自动拆分为若干连续片段。”
2. “预计生成 2 段，最后合成为 30 秒成片。”
3. “系统会自动检查人物、场景和动作衔接，必要时补一个短过渡画面。”
...
- 18 个叙事点、30 秒内容默认被分成 2 个 15 秒生成片段；只有明确场景边界或能力上限变化才增加片段。
- 所有叙事点恰好归属一个生成片段。
- 片段时长总和严格等于目标总时长。
- 实际 Provider TaskRun 数量等于 GenerationSegment 数量。
- 片段依赖顺序和交接帧顺序正确。
- 语义衔接结果和自动修复次数可追溯；每个边界最多一次自动修复。
- 自动转场不改变用户看到的主生成片段数量或本地 Provider TaskRun 数量，并保持目标时长容差。
- 失败片段可以单独重试。
...
> **2026-09-01 当前音频口径**：自动视频使用 Provider 原生音频，只有操作者明确选择时才使用服务端 Doubao 替换；不要求用户上传旁白/样音。`NARRATION_SAMPLE`/`USER_SOURCE_AUDIO` 仅为兼容与隔离测试角色。本文旧的音频输入或 Provider 禁用叙述均视为历史/默认 CI 边界，当前实现以《自动生成音频与视频匹配正式使用开发文档》为准。
