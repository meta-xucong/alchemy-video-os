...
- （历史兼容）轻量媒体服务费复用 Sub2API 现有 `usage_logs`，通过受保护 `GET /api/veyra/internal/users/{user_id}/usage/{request_id}` 读取已结算 `actual_cost`；Worker 再按冻结的 `actual_cost × 0.20 + 1` 计算 Video OS 额外费用。共享 Provider Key 的部署可用私有 `SUB2API_VIDEO_USAGE_USER_ID` 指定只读 usage 所属账户；新任务提交前该账户必须与当前登录账户一致，否则沿用既有 `PROVIDER_REJECTED` fail-closed，不提交 Provider。匹配时任务当前登录用户仍是实际 Video OS debit 目标。该端点是只读事实适配，不新增 Sub2API 账本；Provider 未成功、下载/媒体校验未通过或 usage 未结算时不进入 debit。当前固定档位模式不读取该 usage 作为价格，按《AI企业内容生产平台_固定档位计费与管理员设置控制台开发文档.md》执行。
