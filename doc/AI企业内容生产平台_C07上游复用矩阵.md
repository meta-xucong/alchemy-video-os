...
- 扫描确认 adapter source 不含默认 fetch、env/dotenv、Key、Authorization/Bearer/Cookie、object key 或签名 query；`apps/` 对 adapter 的仅有引用在 Task Worker 测试中，非运行时装配。C07 已由独立审计接受；旧 `IN_PROGRESS` 与 `READY_FOR_AUDIT` 叙述仅为历史审计轨迹。接受后仅允许受限 Git 备份，远端 tag 复核前不得开始 C08。
