...
状态：`SUPERSEDED / HISTORICAL_ONLY`

> **作废说明（2026-09-24）**：本文记录旧设计背景，不再授权产品实现。旧方案中的 deterministic 资料分类、关键词/二元词评分、固定事实条数、按段本地检索和“一键自动批准”语义已被撤销。当前唯一生效方案是 `AI企业内容生产平台_反自造逻辑治理与源仓库收敛完整优化方案.md`：平台只保留结构索引与 locator，完整 Markdown 和任务上下文交给 Semantic Director；用户须显式确认分镜、交付与音乐模式。
