...
## 2026-09-17 导演式语义分段 source ownership 审计补记（历史记录：当时 ACCEPTED；现 `HISTORICAL_SUPERSEDED`；仅本窄章节）

> 本条保留当时的固定来源、测试计数和审计事实，不再授权 `source_ownership/source_spans`、UTF-16 偏移或连续覆盖作为 LLM 输出协议。当前自然语言导演分段边界以《AI企业内容生产平台_原仓库自然语言导演分段与口播保真开发文档.md》为准；不改变正式总账或全平台状态。
