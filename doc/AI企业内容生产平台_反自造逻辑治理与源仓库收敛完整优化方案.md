...
状态：`IMPLEMENTED / LOCAL_ACCEPTED / PRODUCTION_DATA_INVENTORY_PENDING`
...
WP-00 至 WP-12 已按依赖顺序完成本地实现、逐包审计和离线回归，当前代码可按本方案进入本地验收。核心结果：

- 真实创作链只接受完整 Canonical Source Bundle 和带证据的 Semantic Director Decision；缺少 LLM、证据、用户决定或 Provider 能力时 fail-closed；
- 文档关键词分类、二元词 overlap、固定权重与固定条数事实选择已从生产包删除；
- 参考图用途、台词、分段、镜头和对象连续性不再由真实路径的关键词/正则重新推断；
- Mock planner/compiler、handoff fixture 与兼容执行器已物理隔离，真实 Worker 只加载 semantic-only 子入口；
...
- 跨商品、剧情、工业、教育、金融/法律、抽象艺术以及中英混合语料的通用性回归通过。

最终本地门禁：`contracts:generate`、18 项目 typecheck、全仓 build、`756 passed / 20 skipped / 0 failed` 的串行 workspace 测试、Media Runtime `152 passed + 7 subtests`、Python compile 和 `git diff --check=0` 全部通过。20 个 skip 均为 PostgreSQL、BullMQ、MinIO 等未配置环境门，不计作行为通过。

本次验收不包含真实 Provider、真实 Pixabay 网络、生产数据库、VPS 或历史 ALCHMED 数据盘点，也没有 commit、push 或部署。

## 1. 总目标

本次优化不是局部替换几个正则，也不是再造一套更复杂的结构化算法，而是重建清晰的语义所有权：

> 真实模式下，所有需要理解自然语言、图片用途、叙事关系、创作意图或内容相关性的决定，只能由看到完整上下文的 LLM 作出；平台确定性代码只负责保存事实、验证证据、执行协议、保证顺序、限制能力、恢复任务，并在无法证明时 fail-closed。
...
- 原始 source text，byte-for-byte 保存；
- 用户明确设置和批准状态；
...
### 4.3 Evidence-Carrying Decision

LLM 输出采用最小、通用、可选字段，而不是固定行业本体。每项决定至少携带：
...
Verifier 不得判断“这句话是不是动作”“这张图是不是人物图”“这个事实是不是重要”。
...
- 用户 source 在任何出站前保持 byte-for-byte；
- 没有用户动作时没有 APPROVED；
...
- 最终 Provider snapshot 中每段事实都能追溯；
- source byte-for-byte 不变；
- 不含 sentinel、内部字段 prose、重复全文或第二次语义推断；
...
| `packages/domain/src/narrative-events.ts` | ACTION/STATE/CONTROL 等关键词分类退出真实路径 |
| `packages/domain/src/reference-roles.ts` | 角色词表、编号附近文本、文件名和 remainder 推断退出真实路径 |
| `packages/domain/src/visual-object-locks.ts` | 固定道具、左右手和 transfer 正则退出真实路径 |
...
| 位置 | 目标 |
| --- | --- |
| `packages/creative-planning/src/index.ts` | 拆成 LLM plan verifier、Mock fixture 和 pure projection；真实入口不再调用 deterministic compiler |
| `apps/workflow-worker/src/semantic-planning-client.ts` | 升级为统一 Semantic Director 适配器，输入完整 source bundle，输出 evidence-backed decisions |
| `apps/workflow-worker/src/execution-service.ts` | 只编排读取、LLM 调用、验证和持久化，不再重分段或二次选择事实 |
...
- source text 和 exact dialogue 不静默改写；
- 文档事实保留 locator；
- reference order 不重排；
- Provider prompt 不包含无证据事实；
- downstream 不重新解释 upstream prose。
## 10. 当前基线与工作区保护

本方案基于 2026-09-24 当前工作区审计：

- 分支 `main`；HEAD `36a7ceba08f2c62fbf9f78e77e56c6973ce5f07c`；
- 工作区此前已有多项 tracked 修改和一个 tracked 删除；
- 两份审计/开发文档及本方案是未跟踪文档；
- 已有定向测试基线为 `378 passed / 17 skipped / 0 failed`；
- 绿测不代表 P0/P1 已解决。

后续每个工作包开始和结束时都必须比较工作区差异集合。不得覆盖、回滚或重新归因既有修改；不得在未授权情况下 commit、push、合并、部署、改数据库或调用真实 Provider。
...
> 结构只用于传递和验证；理解由 LLM 完成；决定有证据；用户有控制权；未知保持未知；平台不再以关键词、正则、评分、默认值或占位符冒充智能。

本方案当前仅为文档设计。未开始修改业务代码、数据库、公共契约、GitHub 或 VPS，也未调用真实 Provider。