...
## 10. 2026-09-24 凭据暴露处置基线

仓库审计发现旧主线末次提交曾跟踪一个本地 Control API 启动脚本，其中包含外部服务凭据。处置遵循“先阻断继续暴露、再净化历史、最后轮换上游凭据”的顺序：
...
外部账号所有者仍须轮换 SUB2API 视频密钥、参考图视觉服务密钥和参考图交付签名密钥，复核暴露窗口内的调用日志，并保留轮换时间戳/审计记录。完整记录见 `AI企业内容生产平台_20260924凭据暴露处置记录.md`。在轮换证明提交前，安全验收不得标记为完全关闭。
