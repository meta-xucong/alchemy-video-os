...
当前使用补充（2026-09-01）：本地 MVP 的默认/CI 语义和上述 Mock 配置不变。自动旁白场景不要求用户上传旁白或样音；在用户明确授权的本机对照中，音频由 Grok native 或显式 Doubao 生成，随后按最新《AI企业内容生产平台_自动生成音频与视频匹配正式使用开发文档.md》做技术检查和人工试听。通用参考图、资料、Logo、MUSIC 上传仍属于本 MVP/平台资产能力；不要把该补充解释为默认开启真实 Provider。

现行实际操作补充（2026-09-01）：用户已授权在本机把视频模式显式切换为 `VIDEO_PROVIDER=sub2api`（Aiself Grok，优先保留 native 音频），需要替换时显式调用 Doubao `seed-tts-2.0`/`zh_female_meilinvyou_uranus_bigtts`。该操作配置只存在未入库的 `.env.local`/进程环境，不改变本地 MVP/CI 的 `VIDEO_PROVIDER=mock` 默认，也不开放 Veyra、共享积分或部署。
