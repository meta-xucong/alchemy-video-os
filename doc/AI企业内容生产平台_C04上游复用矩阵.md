...
| Studio 工作台交互与响应式视觉语法 | 本机 Alchemy Media `src_skeleton/app/static/index.html`、`styles.css`、`app.js:publicSafeErrorText`、`renderV3ProjectNextActions` | 保留紧凑 operator workspace、显式创建/恢复动作、公共错误降噪、桌面主区加侧栏和移动单列收敛的交互原则 | `doc/AI企业内容生产平台_Studio前端功能与交互设计.md`、`apps/studio-web/app/pages/index.vue`、`apps/studio-web/app/assets/studio.css` | 不复制 Alchemy HTML/CSS/JS、认证/Cookie、Provider 路由、JSONL、资产路径、账本、运行时全局状态或媒体。Studio 仍只使用既有 `/api/v1` 和 public SSE；自动化验证页面控件、上传/刷新预览、Mock 生成/重试、响应式和敏感字段拒绝。 |
