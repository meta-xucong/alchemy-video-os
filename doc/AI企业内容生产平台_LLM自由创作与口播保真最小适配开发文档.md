...
> 文档状态：`HISTORICAL_PARTIAL_SUPERSEDED`（LLM 输出/ownership 部分已由 2026-09-17 新文档取代；其余口播、引用、预算和平台边界仅作可复用历史规则）
>
...
> 本文中“生产 LLM 必须遵循私有 `source_ownership/source_spans` envelope”的表述已由《AI企业内容生产平台_原仓库自然语言导演分段与口播保真开发文档.md》superseded。本文保留口播逐字保真、引用顺序、预算和平台边界等可复用规则；当前生产 LLM 不再被要求返回 ownership、span、beat 或 motion 等创作结构。
