...
> **2026-09-01 当前口径覆盖**：自动视频不要求用户上传旁白、样音或其它 spoken audio；优先使用已认证 Provider 原生音频，替换时才由服务端显式 Doubao 生成。默认/CI 仍使用 Mock；本轮明确授权的本机 Aiself Grok/Doubao 对照以《AI企业内容生产平台_自动生成音频与视频匹配正式使用开发文档.md》为唯一执行入口。本文旧的上传音频或“统一 Piper”描述均为历史/兼容，不授权新增逻辑。
