...
3. Worker 通过注入的 `VideoUsagePort` 按“Provider API Key 所属账户（配置了 `SUB2API_VIDEO_USAGE_USER_ID` 时）或任务 `external_user_id`（未配置时）+ provider_request_id”读取唯一真实 usage 事实，并校验返回模型与快照模型一致。配置共享 owner 时，新任务还必须满足 owner 与任务登录账户相同，否则在 Provider 提交前以既有 `PROVIDER_REJECTED` 安全拒绝；匹配时 Video OS debit 目标仍是任务登录用户。
...
### 2026-09-16 共享 Provider Key 的用量归属修正

VPS 实测发现，Provider API Key 可以属于一个固定的 Sub2API/Veyra 账户，而 Video OS 登录用户是另一个账户：视频已经成功生成并下载，但按登录用户查询 usage 会得到“usage 尚未结算/不存在”，任务会一直停在 `BILLING_PENDING`。因此 Worker 支持私有环境变量 `SUB2API_VIDEO_USAGE_USER_ID`：它标识共享 Provider Key 所属账户，用于读取该账户的已结算 `actual_cost`；在新任务提交前，Worker 还要求它与当前任务冻结的 `external_user_id` 一致，发现不一致即以既有 `PROVIDER_REJECTED` fail-closed，不向 Provider 提交，避免把原生费用静默扣到另一个账户。匹配后，Video OS 的服务费 debit 仍发给当前登录用户。未配置时保持原有“按任务用户读取 usage”的行为，适用于每个登录账户使用其自身 Provider Key 的部署。该变量不进入公开 API、任务快照、事件、日志或浏览器，也不修改 Sub2API 的原生扣费。要支持多个登录账户，必须为每个账户提供对应的服务端 Provider Key/上游委托能力；Video OS 不会把一个共享 Key 的原生费用伪装转移给其他账户。
