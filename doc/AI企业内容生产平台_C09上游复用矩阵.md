...
| 视频真实用量读取 | Sub2API `backend/internal/service/usage_service.go` 的 `ListWithFilters` 与 `backend/internal/veyra/routes.go` 的受保护 internal 路由（`custom/main` 工作树） | `packages/credit-veyra/src/video-usage.ts`、`packages/domain/src/video-billing.ts` | `user_id + request_id + model + actual_cost` 只读事实；Grok 既有 `grok-video:` 稳定键 | 不复制 `usage_logs`、余额更新或第二账本；未结算返回重试，不用时长/固定价估算 | Veyra route fixture、adapter envelope/identity/retry tests |
