...
状态：`IN_PROGRESS / WP-00`

## 1. 基线

- 工作区：`D:\AI\alchemy_video_OS`
- 分支：`main`
- HEAD：`36a7ceba08f2c62fbf9f78e77e56c6973ce5f07c`
- 与 `origin/main`：`+0/-0`
- 开工前已有差异：14 个 tracked 修改、1 个 tracked 删除、3 份未跟踪治理文档。
- 开工前定向基线：`378 passed / 17 skipped / 0 failed`。
- 本轮禁止：commit、push、tag、部署、数据库生产迁移、真实 Provider/TTS/Veyra、VPS、网络调用。
...
| 工作包 | 状态 | 审计证据 |
| --- | --- | --- |
| WP-00 | `ACCEPTED` | P0=12、P1=16；总控/ADR/章节账本交叉引用；`git diff --check=0` |
| WP-01 | `ACCEPTED` | 止血项完成；672 passed / 13 skipped / 0 failed；`git diff --check=0` |
| WP-02 | `ACCEPTED` | Canonical Source Bundle、EvidenceRef、Semantic Director Decision 契约；48 passed / 0 failed；build 与 diff check 通过 |
| WP-03 | `ACCEPTED` | Canonical builder、纯证据 verifier、Verified Director、OpenAI-compatible client；Contracts 49 + Planning 100 + Workflow 43 全通过 |
| WP-04 | `ACCEPTED` | Structural index only；真实入口无 deterministic 文档分类/事实排名；247 pass / 13 skip / 0 fail；5 包 typecheck 与 diff check 通过 |
| WP-05 | `ACCEPTED` | 参考图客观分析与用途解耦；直生成只信显式 binding；自动生产只信带证据的 Semantic Reference Projection；canonical order 全链路冻结；527 pass / 18 skip / 0 fail |
| WP-06 | `ACCEPTED` | exact dialogue 由 Semantic Director 单一拥有；Provider/Control API/Composition 不再重解析真实 brief prose；Mock 与历史读取明确隔离；504 pass / 18 skip / 0 fail |
| WP-07 | `ACCEPTED` | 真实分段、时长、视觉表达与参考用途只来自 verified semantic decision；真实 Worker 不实例化 deterministic planner/compiler；504 pass / 18 skip / 0 fail |
| WP-08 | `ACCEPTED` | 创作字段可选、无 sentinel；Provider compiler 为 pure projector；source 不压缩，exact dialogue/reference/caption 超限直接 PROMPT_BUDGET；504 pass / 18 skip / 0 fail |
| WP-09 | `ACCEPTED` | 新命令显式要求分镜/交付/音乐决定；Studio 两次独立人工确认；无自动审批、无隐式 AUTO、无伪 budget=0；整套 BGM 匹配登记为用户授权 PLATFORM_OWNED；278 pass / 13 skip / 0 fail |
| WP-10 | `ACCEPTED` | 无真实 evaluator 时连续性保持 UNAVAILABLE 且零 repair/frame work；无伪 succeeded/ACCEPTED；Final Review 未检查项=null/UNAVAILABLE，固定 CLIP 与手调台词阈值不参与验收；393 pass / 12 skip / 0 fail，另 7 subtests pass |
| WP-11 | `ACCEPTED` | 真实 Worker 只加载 semantic-only 子入口；Mock executor/client 与 fixture 迁入 mock/tests；文档关键词/二元词/评分器及 legacy fact store 删除；固定 CLIP/台词阈值退役；新 DeliveryPlan 只写完整 AudioPlan/ALCHMED8，ALCHMED1-7 只读并等待授权盘点；定向 365 pass / 12 skip / 0 fail，另 7 subtests pass |
| WP-12 | `ACCEPTED` | contracts 生成、18 项目 typecheck、全仓 build、串行 workspace 测试 756 pass / 20 skip / 0 fail、Media Runtime 152 pass + 7 subtests、Python compile、跨行业/跨语言通用性回归及 `git diff --check=0` 全通过 |
