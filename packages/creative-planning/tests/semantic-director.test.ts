...
test("verifier accepts a complete evidence-backed semantic decision", () => {
  const verified = verifySemanticDirectorDecision(bundle, readyDecision());
...
test("verifier rejects source, document, reference, and user-decision evidence mismatches", () => {
  ...
  assert.throws(() => verifySemanticDirectorDecision(bundle, sourceMismatch),
  ...
  assert.throws(() => verifySemanticDirectorDecision(bundle, documentMismatch),
  ...
  assert.throws(() => verifySemanticDirectorDecision(bundle, referenceMismatch),
  ...
  assert.throws(() => verifySemanticDirectorDecision(bundle, userMismatch),
...
test("verifier preserves canonical reference order and provider duration bounds", () => {
  ...
  assert.throws(() => verifySemanticDirectorDecision(bundle, reordered),
  ...
  assert.throws(() => verifySemanticDirectorDecision(twentySecondBundle, overlong),
...
test("blocked decisions remain inspectable but cannot enter execution", () => {
  const blocked = verifySemanticDirectorDecision(bundle, {
    version: 1,
    source_hash: bundle.source_hash,
    target_duration_seconds: 10,
...
test("verified director never falls back when the client fails or returns malformed facts", async () => {
  const unavailable = new VerifiedSemanticDirector({
    async decide() { throw new Error("offline"); },
  });
  await assert.rejects(() => unavailable.decide(bundle), /offline/);

  const malformed = new VerifiedSemanticDirector({
...
test("projects only the verified segment reference order and Provider roles", () => {
  const verified = verifySemanticDirectorDecision(bundle, readyDecision());
