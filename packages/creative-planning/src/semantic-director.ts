...
/**
 * Verify schema, immutable identities, hashes, exact source spans, ordering,
 * duration bounds, and reference scope.
 *
 * This is a provenance/structure check only. It does not prove that a visual
 * decision is semantically entailed by its citations, that every meaningful
 * source fact was covered, or that the resulting video is visually correct.
 */
export const verifySemanticDirectorProvenance = (
  bundleInput: unknown,
...
export const projectSemanticDialogues = (
  ...
  if (!segment) {
    return invalid("SEMANTIC_DECISION_MALFORMED", "Semantic segment is missing from the provenance-checked decision.");
  }
  ...
  const dialogues = segment.dialogue_ids.map((dialogueId) => {
    ...
    if (!dialogue) {
      return invalid("SOURCE_EVIDENCE_INVALID", "A segment dialogue has no provenance-checked exact-text decision.");
    }
    return {
...
export const projectSemanticReferences = (
  ...
  const references = segment.reference_asset_ids.map((assetId) => {
    ...
    if (!usage) {
      return invalid("REFERENCE_EVIDENCE_INVALID", "A segment reference has no provenance-checked usage decision.");
    }
    return {
      asset_id: assetId,
