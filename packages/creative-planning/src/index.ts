...
const readLlmSegmentDecisions = (
  ...
  decisions.forEach((decision, index) => {
    ...
    const lines = decision.dialogue_line_sequences.map((sequence) => context.dialogueLines[sequence - 1]!);
    const requiredSeconds = dialogueDurationSeconds(lines);
    if (requiredSeconds > decision.duration_seconds) {
      malformed(`Segment ${index + 1} does not have enough time for its authored dialogue.`);
    }
  });
  return decisions;
};
...
const buildLlmSegmentedDraft = (
  ...
  const segmentBeats = decisions.map((decision, index) => {
    const sentences = extractNarrativeSentences(decision.visual_prompt)
      .filter((sentence) => sentence.kind !== "CONTROL" && !authoringControlSentence.test(sentence.text))
      .map((sentence) => concise(sentence.text, ""))
      .filter(Boolean);
    const events = sentences.length > 0 ? sentences : [concise(decision.visual_prompt, "本段视觉推进")];
    return { sequence: index + 1, events };
  });
  const beats: PlannedScriptBeat[] = [];
  let beatSequence = 1;
  for (const segment of segmentBeats) {
    for (const event of segment.events) {
      const primaryEvent = concise(stripSpokenDialogue(event), "故事推进");
      beats.push({
        sequence: beatSequence,
        title: `叙事点 ${beatSequence}：${primaryEvent}`,
  ...
  const shotSpecs = decisions.map((decision, index) => {
    ...
    const segmentEvents = segmentBeats[index]!.events;
    const segmentBeatSequences = beats
      .filter((beat) => beat.generationSegmentSequence === sequence)
      .map((beat) => beat.sequence);
    const segmentHasExplicitSceneChange = hasExplicitSceneChangeSignal(decision.visual_prompt);
    ...
    const primaryEvent = concise(segmentEvents[0]!, "故事推进");
    const startState = sequence === 1 ? "故事开场状态" : `承接第 ${sequence - 1} 段的结束状态`;
    const endState = sequence === decisions.length ? "完整故事目标完成" : `为第 ${sequence + 1} 段建立可见过渡`;
    const transitionSummary = sequence === 1 ? "建立故事开场画面与核心人物状态。" : `从第 ${sequence - 1} 段的结束动作或场景状态继续。`;
    const narrativeGoal = selectedDialogueLines.length > 0
      ? selectedDialogueLines.map((line) => `说：“${line}”`).join("；")
      : concise(decision.visual_prompt, primaryEvent);
    const voicePerformance = voicePerformanceForLines(selectedDialogueLines);
    const cameraShot = cameraShotFor({
      sequence,
      count: decisions.length,
      durationSeconds: decision.duration_seconds,
      narrativeIntent: primaryEvent,
    ...
    const motion = createMotionPlan({
      sequence,
      durationSeconds: decision.duration_seconds,
      events: segmentEvents.map(stripSpokenDialogue).filter(Boolean),
    ...
    return {
      ...
      // Dialogue-bearing segments need the director's visual supplement in
      // generatedPromptParts. A visual-only segment already carries its
      // supplement in narrativeGoal, so do not inject it twice.
  ...
  const documentContextCount = (input.documentContexts ?? []).filter((contextItem) => Boolean(normalize(contextItem.content))).length;
  const continuityLevel: ContinuityLevel = input.sourceAssetIds.length > 0 && shotSpecs.length > 1 ? "REVIEW_REQUIRED" : "STANDARD";
  const firstEvent = segmentBeats[0]?.events[0] ?? rawSourceText;
  return {
    plannerVersion: DETERMINISTIC_PLANNER_VERSION,
    beats,
    title: `故事计划：${concise(stripSpokenDialogue(firstEvent), "故事计划")}`,
    summary: `共 ${beats.length} 个叙事点，由自然语言导演规划为 ${shotSpecs.length} 个生成片段，总时长 ${input.targetDurationSeconds} 秒。${factContextCount > 0 ? ` 已冻结 ${factContextCount} 条项目事实以约束事实一致性。` : documentContextCount > 0 ? ` 已关联 ${documentContextCount} 份已整理资料以约束事实一致性。` : ""}${normalize(input.stylePreferences) ? ` 风格偏好：${concise(input.stylePreferences, "")}` : ""}`,
    totalDurationSeconds: input.targetDurationSeconds,
    continuityLevel,
