/**
 * MOCK_ONLY / LEGACY_COMPATIBILITY planning implementation.
 *
 * This module may be loaded only after VIDEO_PROVIDER=mock is resolved. It is
 * deliberately absent from the production-safe package root.
 */
...
const containsForeignSemanticSource = (
  visualPrompt: string,
  forbiddenSource: readonly string[],
) => {
  const normalizedPrompt = normalize(visualPrompt);
...
const readLlmSegmentDecisions = (
  ...
  const decisions = value.map((item, index) => {
    ...
    if ((record.visual_prompt as string).includes("__MOCK_UNSPECIFIED_")) {
...
const containsMockPlaceholder = (value: unknown): boolean => {
  if (typeof value === "string") return value.includes("__MOCK_UNSPECIFIED_");
  if (Array.isArray(value)) return value.some(containsMockPlaceholder);
  if (value && typeof value === "object") return Object.values(value).some(containsMockPlaceholder);
  return false;
};
...
const MOCK_UNSPECIFIED_SUBJECT = "__MOCK_UNSPECIFIED_SUBJECT";
const MOCK_UNSPECIFIED_CAMERA_SIZE = "__MOCK_UNSPECIFIED_CAMERA_SIZE";
const MOCK_UNSPECIFIED_CAMERA_ANGLE = "__MOCK_UNSPECIFIED_CAMERA_ANGLE";
const MOCK_UNSPECIFIED_CAMERA_MOVEMENT = "__MOCK_UNSPECIFIED_CAMERA_MOVEMENT";
const MOCK_UNSPECIFIED_CAMERA_DIRECTION = "__MOCK_UNSPECIFIED_CAMERA_DIRECTION";
const MOCK_UNSPECIFIED_OPENING_STATE = "__MOCK_UNSPECIFIED_OPENING_STATE";
const MOCK_UNSPECIFIED_CLOSING_STATE = "__MOCK_UNSPECIFIED_CLOSING_STATE";
const MOCK_UNSPECIFIED_TRANSITION_IN = "__MOCK_UNSPECIFIED_TRANSITION_IN";
const MOCK_UNSPECIFIED_TRANSITION_OUT = "__MOCK_UNSPECIFIED_TRANSITION_OUT";
const MOCK_UNSPECIFIED_SCENE_LOCK = "__MOCK_UNSPECIFIED_SCENE_LOCK";
const MOCK_UNSPECIFIED_CONTINUITY = "__MOCK_UNSPECIFIED_CONTINUITY";
const MOCK_UNSPECIFIED_COMPLEXITY_SCORE = 0;
...
const buildLlmPlatformMotionPlan = (input: {
  ...
  try {
    ...
    scene_lock: MOCK_UNSPECIFIED_SCENE_LOCK,
    character_locks: [],
    prop_locks: [],
    ...(input.keyVisualObjects.length > 0 ? { key_visual_objects: [...input.keyVisualObjects] } : {}),
    motion_beats: [{
      ...
      subject_refs: [MOCK_UNSPECIFIED_SUBJECT],
      start_pose: MOCK_UNSPECIFIED_OPENING_STATE,
      end_pose: MOCK_UNSPECIFIED_CLOSING_STATE,
      shot_size: MOCK_UNSPECIFIED_CAMERA_SIZE,
      camera_movement: MOCK_UNSPECIFIED_CAMERA_MOVEMENT,
      continuity_locks: [],
      prohibited_changes: [],
    ...
    opening_state: MOCK_UNSPECIFIED_OPENING_STATE,
    closing_state: MOCK_UNSPECIFIED_CLOSING_STATE,
    transition_in: MOCK_UNSPECIFIED_TRANSITION_IN,
    transition_out: MOCK_UNSPECIFIED_TRANSITION_OUT,
    // Required by the existing shape; never interpreted as a director score.
    complexity_score: MOCK_UNSPECIFIED_COMPLEXITY_SCORE,
    source_narrative_beat_sequences: [sequence],
  });
  } catch (error) {
    throw new LlmSemanticPlanningError(
...
const buildLlmSegmentedDraft = (
  ...
  const shotSpecs = decisions.map((decision, index) => {
    ...
    const cameraShot: CameraShotSpec = {
      ...
      shotSize: MOCK_UNSPECIFIED_CAMERA_SIZE,
      cameraAngle: MOCK_UNSPECIFIED_CAMERA_ANGLE,
      primaryMovement: MOCK_UNSPECIFIED_CAMERA_MOVEMENT,
      movementDirection: MOCK_UNSPECIFIED_CAMERA_DIRECTION,
      narrativeIntent: decision.visual_prompt,
      openingState: MOCK_UNSPECIFIED_OPENING_STATE,
      closingState: MOCK_UNSPECIFIED_CLOSING_STATE,
      transition: MOCK_UNSPECIFIED_TRANSITION_OUT,
    };
  ...
  return {
    plannerVersion: "LEGACY_MOCK_LLM_FREEFORM_PLANNER",
    beats,
    title: "故事计划",
...
export class DeterministicStoryboardCompiler implements StoryboardCompilerPort {
  async compile(input: CompilationInput): Promise<CompiledPromptPackage> {
    ...
    const unsupportedInternalShape = containsMockPlaceholder({
    ...
    const cameraInstruction = input.cameraShot && !containsMockPlaceholder(input.cameraShot)
    ...
    const motionInstruction = motionPlan && !containsMockPlaceholder(motionPlan)
    ...
    if (containsMockPlaceholder(sourcePrompt)) {
    ...
    const generatedPromptParts = [
      ...
      .filter((part) => !containsMockPlaceholder(part))
      .filter(Boolean);
    ...
    return {
      ...
      prompt,
      visualConstraints: {
        ...(containsMockPlaceholder(input.startState) ? {} : { start_state: input.startState }),
        ...(containsMockPlaceholder(input.endState) ? {} : { end_state: input.endState }),
        ...(containsMockPlaceholder(input.continuityNote) ? {} : { continuity_note: input.continuityNote }),
      ...
      capabilitySnapshot: {
        ...
        ...(containsMockPlaceholder(input.cameraShot) ? {} : { camera_shot: input.cameraShot }),
