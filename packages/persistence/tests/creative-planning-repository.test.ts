    ...
    shotSpecs: [
      {
        ...
        dependsOnSequences: [],
        dialogueLines: ["Deep night is not the end; it is the beginning of protection."],
...
test("C11 keeps story planning immutable, workspace-scoped, and free of execution side effects", async () => {
  ...
  assert.deepEqual(storyboard.shotSpecs[0]?.dialogueLines, ["Deep night is not the end; it is the beginning of protection."]);
  assert.deepEqual(storyboard.shotSpecs[1]?.dialogueLines, []);
  assert.deepEqual(storyboard.shotSpecs[2]?.dialogueLines, []);
